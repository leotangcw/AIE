...
app = FastAPI(
    title="AIE Desktop API",
    description="AIE backend API",
    version="0.2.0",
    lifespan=lifespan,
)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if not is_local:
        ...
        if pw_hash:
            token = websocket.query_params.get("token") or websocket.cookies.get("AIE_token")
...
logger.info(f"Checking frontend dist: {frontend_dist}")
