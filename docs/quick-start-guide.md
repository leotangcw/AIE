...
> 本文档详细说明 AIE 各项核心功能的配置方法，包括具体的文件路径、配置格式和获取密钥的步骤。按照本指南操作即可逐步开通所有功能。
...
这是 AIE 运行的前提条件。没有大模型 API，所有 AI 功能都无法使用。
...
渠道是 AIE 连接外部即时通讯平台的桥梁。配置渠道后，你可以通过飞书、钉钉、Telegram、QQ、微信等平台与 AIE 对话。
...
技能（Skills）是 AIE 的可插拔功能模块，每个技能有独立的配置文件。技能目录位于项目根目录的 `skills/` 文件夹下。
...
5. 配置令牌：
   - **Token name**：随意命名（如 `AIE Pages Deploy`）
   - **Permissions**（权限）：
...
定时任务管理技能通过命令行管理 AIE 的定时任务系统，支持完整的 CRUD 操作和会话数据管理。
...
如果你需要通过局域网或公网 IP（非 `127.0.0.1`）访问 AIE，系统会自动启用远程访问认证保护。
...
1. 通过远程 IP 访问 AIE（如 `http://192.168.1.100:8000`）
