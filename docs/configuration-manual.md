...
> 本手册覆盖 AIE 所有配置项的完整说明，包括数据流向、生效范围和验证方法。适用于管理员和开发者。
...
AIE 内置远程访问认证模块，当通过非本地 IP 访问时自动触发认证保护。
...
- 内存存储，24 小时过期
- 应用重启后所有 session 失效
- 支持 Cookie（`AIE_token`）和 Authorization Header（`Bearer xxx`）两种传递方式
...
| Token 失效 | 清除 localStorage 中的 `AIE_token` | 下次 API 请求返回 401，跳转登录页 |
